...
13. **Document every completed job for the next agent** — at the end of any bug fix, feature, or investigation session, write a session doc under `docs/SESSION_<date>_<short-topic>.md` covering: the symptom, the full root-cause chain (not just the fix), every file touched and why, what was verified (tests/typecheck output) vs what still needs a real device/build to confirm, and any lesson a future agent should know before touching the same area again (e.g. "don't rename X, Y imports it by exact name"). Add a one-line pointer to it in memory (`MEMORY.md` + a linked memory file) so future sessions inherit the finding without re-deriving it. This applies even to fixes that feel small — the goal is that no bug gets rediscovered from scratch.
