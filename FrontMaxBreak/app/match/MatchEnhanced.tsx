...
export default function MatchEnhanced() {
    ...
    try {
      ...
      if (matchDetails.frame_scores && matchDetails.frame_scores.trim()) {
        ...
        if (parsedFrameScores.length > 0) {
          ...
          // Enhanced fallback to legacy parsing methods if HTML parser fails
          try {
            ...
            // If frame_scores is not JSON, try various string formats
            let frameStrings: string[] = [];
            
            // Try semicolon-separated first
            if (matchDetails.frame_scores.includes(';')) {
              frameStrings = matchDetails.frame_scores.split(';').filter(f => f.trim());
            } 
            // Try comma-separated with session handling
            else if (matchDetails.frame_scores.includes(',')) {
              // Remove session headers and clean up
              let cleanString = matchDetails.frame_scores
                .replace(/Session\s+\d+:/gi, '')  // Remove session headers
                .replace(/<br\s*\/?>/gi, ',')     // Replace <br> with commas
                .replace(/<[^>]*>/g, '')          // Remove other HTML tags
                .trim();
              frameStrings = cleanString.split(',').filter(f => f.trim());
            }
      ...
      // Final validation of parsed scores
      if (parsedFrameScores.length > 0) {
        // Verify that parsed scores match the match totals
        const p1ScoreFromFrames = parsedFrameScores.filter(f => f.winner === 1).length;
        const p2ScoreFromFrames = parsedFrameScores.filter(f => f.winner === 2).length;
        
        if (p1ScoreFromFrames !== player1Score || p2ScoreFromFrames !== player2Score) {
          logger.warn(`[MatchEnhanced] Frame score mismatch detected. Expected: ${player1Score}-${player2Score}, Got: ${p1ScoreFromFrames}-${p2ScoreFromFrames}. Using fallback.`);
          parsedFrameScores = []; // Clear invalid parsed scores to trigger fallback
        }
      }
  ...
  const loadData = useCallback(async (refreshing = false) => {
    ...
    try {
      // Add safety check for invalid player IDs
      logger.debug(`[MatchEnhanced] Loading match details for API Match ID: ${apiMatchId}`);
      ...
      // CRASH FIX: Robust player ID validation to prevent app crashes
      // Handle cases where backend returns null/undefined player IDs
      let safePlayer1Id = matchDetailsTyped.player1_id;
      let safePlayer2Id = matchDetailsTyped.player2_id;
      
      // Validate and fix player1_id
      if (safePlayer1Id === null || safePlayer1Id === undefined || isNaN(safePlayer1Id) || safePlayer1Id <= 0) {
        logger.warn(`[MatchEnhanced] CRASH PREVENTION: Invalid player1_id: ${matchDetailsTyped.player1_id}, using fallback`);
        safePlayer1Id = 376; // UNKNOWN_PLAYER_ID constant
      }
      
      // Validate and fix player2_id
      if (safePlayer2Id === null || safePlayer2Id === undefined || isNaN(safePlayer2Id) || safePlayer2Id <= 0) {
        logger.warn(`[MatchEnhanced] CRASH PREVENTION: Invalid player2_id: ${matchDetailsTyped.player2_id}, using fallback`);
        safePlayer2Id = 376; // UNKNOWN_PLAYER_ID constant
      }
      
      // Create safe match details with guaranteed valid player IDs
      const safeMatchDetails = {
        ...matchDetailsTyped,
        player1_id: safePlayer1Id,
        player2_id: safePlayer2Id,
        // Also ensure player names exist to prevent display issues
        player1_name: matchDetailsTyped.player1_name || 'Player 1',
        player2_name: matchDetailsTyped.player2_name || 'Player 2'
      };
      ...
      if (matchDetailsTyped.api_match_id) {
        logger.log(`[MatchEnhanced] 🔄 Syncing fresh match data to tournament cache for consistency`);
        syncMatchDataToTournamentCache(matchDetailsTyped);
        
        logger.log(`[MatchEnhanced] 🧹 Clearing tournament caches to prevent stale score display in production builds`);
        apiCache.clearAllTournamentMatches();
      }
      ...
      if (needsH2H) {
        ...
        if (
          ...
          matchDetailsTyped.player1_id !== 376 && // UNKNOWN_PLAYER_ID
          matchDetailsTyped.player2_id !== 376 && // UNKNOWN_PLAYER_ID
          matchDetailsTyped.player1_id > 0 &&
          matchDetailsTyped.player2_id > 0
        ) {
          loadH2HData(matchDetailsTyped.player1_id, matchDetailsTyped.player2_id);
        } else {
          logger.debug(`[MatchEnhanced] Skipping H2H for unknown/invalid players: ${matchDetailsTyped.player1_id}, ${matchDetailsTyped.player2_id}`);
          // Set empty H2H data for unknown players
          setH2hData({
            Player1Wins: 0,
            Player2Wins: 0,
            totalMeetings: 0,
  ...
  }, [matchDetails?.status_code, apiMatchId]); // CRASH FIX: Remove score dependencies to prevent memory leaks
  ...
  }, [apiMatchId]); // CRASH FIX: Remove loadData dependency to prevent infinite loop

  // TEMPORARY: Re-enable unmount sync to understand original issue
  useEffect(() => {
    return () => {
      if (matchDetails) {
        logger.log(`[MatchEnhanced] 🔄 Component unmounting - syncing match data to tournament cache for API ID ${matchDetails.api_match_id}`);
        syncMatchDataToTournamentCache(matchDetails);
      }
    };
  }, [matchDetails]);
